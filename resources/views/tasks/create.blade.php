...
<div class="max-w-4xl mx-auto">
    <div class="mb-5">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('tasks.create') }}</h1>
    </div>
            ...
            <div>
                ...
                >
                    @foreach ($labels as $label)
                        <option value="{{ $label->id }}" {{ (is_array(old('labels')) && in_array($label->id, old('labels'))) ? 'selected' : '' }}>
                            {{ $label->name }}
                        </option>
                    @endforeach
                </select>
